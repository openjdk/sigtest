...
public class BCProcessor extends HumanErrorFormatter {
    ...
    class Rule1_1 extends Handler {
        ...
        boolean acceptMessageList(List<Message> l) {
            ...
            if (m.messageType == MessageType.ADD_METHS && m.errorObject instanceof MethodDescr) {
                ...
                try {
                    if (isDefender(md, m.className)) {
                        return false;
                    }

                    // allow adding methods from Object (see also 2.1)
                    if (fromObject(md)) {
                        return false;
                    }
                ...
                try {
                    if (!isDefender(md, m.className)) {
                        ...
                        if (!extensibleInterfaces && clHier.isInterface(dcn) && !clHier.isAnnotation(dcn)) {
                            ...
                            // allow adding methods from Object
                            if (fromObject(md)) {
                                return false;
                            }
    ...
    private boolean isDefender(MethodDescr md, String className) throws ClassNotFoundException {
        return !md.isAbstract() && clHier.isInterface(className);
    }

    private void initObject() throws ClassNotFoundException {
        if (objectClass == null) {
            objectClass = clHier.load("java.lang.Object");
        }
    }
